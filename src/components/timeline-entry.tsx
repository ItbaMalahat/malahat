...
export function TimelineEntry({
  ...
  return (
    <article className="grid gap-3 border-t border-[var(--border)] py-10 first:border-t-0 first:pt-0 md:grid-cols-[180px_1fr] md:gap-10">
      <p className="text-[13px] font-medium uppercase tracking-[0.1em] text-[var(--muted)] md:pt-1">
        {period}
      </p>
      <div>
        ...
        {highlights && highlights.length > 0 ? (
          <ul className="mt-5 max-w-3xl list-disc space-y-2 pl-5 text-[15px] leading-7 text-[var(--muted)]">
            {highlights.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
